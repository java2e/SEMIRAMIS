...
public class XMLToSistemAktarimDAO {
	...
	public void Kaydet(XMLToUyap xmlVeri, ExcellModel xlsVeri){
		...
		// Gelen veride Müvekkil için sadece Müvekkilin adı alınarak ilgili tablodan id si alınır. Diğer bilgier zaten sistemde mevcuttur.
		AlacakliDAO muvekkil = new AlacakliDAO();
		MuvekkilId = muvekkil.MuvekkilIdGetir(xmlVeri.get__kisiKurumBilgileri2_ad());
		
		// Borçlu bilgisinin tümü alınır. borçlu sistemde kayıtlı değil ise bilgileri sisteme işlenir.
		BorcluBilgisiDAO Borclu = new BorcluBilgisiDAO(); // borçluyu kaydetmek için gerekli DAO ürettik
		
		BorcluBilgisi borcluBilgileri =  new BorcluBilgisi(); // borçlu bilgilerini doldurmak için yeni bir Borçlu Nesnesi Ürettik
		borcluBilgileri.setAdSoyad(xmlVeri.get__kisiKurumBilgileri3_ad());
		
		AdresTuruDAO adresturu = new AdresTuruDAO(); // Adres türüne ait İd bilgisini çekmek için adresDAO ürettik
		...
		try {
			BorcluId = Borclu.saveBorclu(borcluBilgileri); // burada borçlu bilgilerini sisteme kaydettik ve id sini aldık.
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		
		//Banka Vekil Avukat Bilgileri zaten sistemde kayıtlı olduğu için işlen yapılmıyor.
		//--------------------------------------------------------------------------------
		
		
		HesapDAO  hesapListesi = new HesapDAO();
		
		Hesap hesapBilgi = new Hesap(); // Hesap Nesnesi oluşturularak gerekli alanlar işleniyor.
	    ...
	    try {
		HesapId= 	hesapListesi.Kaydet(hesapBilgi); // Hesap Bilgi girişlerini hesap tablosuna işledik.
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
