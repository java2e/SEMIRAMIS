package pelops.model;

import java.util.ArrayList;

public class FreeXML {

	private String dosyaTuru;
	private String takipTuru;
	private String takipYolu;
	private String takipSekli;
	private String alacaklininTalepEttigiHak;
	private String aciklama48e9;
	private String BK84MaddeUygulansin;
	private String BSMVUygulansin;
	private String KKDFUygulansin;
	private String dosyaBelirleyicisi;
	private String dosyaTipi;
	private String digerAlcakAciklama;
	private String paraBirimi;
	private String takipTarihi;
	private String uyapParaTuru;
	 
	private String belgeMiktari;
	private String dovizTipi;
	
	
	private String adSoyad;
	private String adresTuruId;
	private String adres;
	private String ad;
	private String soyad;
	private String tcNo;
	private String ilAdi;
	private String ilId;
	
	private String muvekkilAdi;
	
	
	
	private ArrayList<FreeXMLHesap> hesapListesi;

	
	
	
	
	
	
	public String getMuvekkilAdi() {
		return muvekkilAdi;
	}

	public void setMuvekkilAdi(String muvekkilAdi) {
		this.muvekkilAdi = muvekkilAdi;
	}

	public String getDosyaTuru() {
		return dosyaTuru;
	}

	public void setDosyaTuru(String dosyaTuru) {
		this.dosyaTuru = dosyaTuru;
	}

	public String getTakipTuru() {
		return takipTuru;
	}

	public void setTakipTuru(String takipTuru) {
		this.takipTuru = takipTuru;
	}

	public String getTakipYolu() {
		return takipYolu;
	}

	public void setTakipYolu(String takipYolu) {
		this.takipYolu = takipYolu;
	}

	public String getTakipSekli() {
		return takipSekli;
	}

	public void setTakipSekli(String takipSekli) {
		this.takipSekli = takipSekli;
	}

	public String getAlacaklininTalepEttigiHak() {
		return alacaklininTalepEttigiHak;
	}

	public void setAlacaklininTalepEttigiHak(String alacaklininTalepEttigiHak) {
		this.alacaklininTalepEttigiHak = alacaklininTalepEttigiHak;
	}

	public String getAciklama48e9() {
		return aciklama48e9;
	}

	public void setAciklama48e9(String aciklama48e9) {
		this.aciklama48e9 = aciklama48e9;
	}

	public String getBK84MaddeUygulansin() {
		return BK84MaddeUygulansin;
	}

	public void setBK84MaddeUygulansin(String bK84MaddeUygulansin) {
		BK84MaddeUygulansin = bK84MaddeUygulansin;
	}

	public String getBSMVUygulansin() {
		return BSMVUygulansin;
	}

	public void setBSMVUygulansin(String bSMVUygulansin) {
		BSMVUygulansin = bSMVUygulansin;
	}

	public String getKKDFUygulansin() {
		return KKDFUygulansin;
	}

	public void setKKDFUygulansin(String kKDFUygulansin) {
		KKDFUygulansin = kKDFUygulansin;
	}

	public String getDosyaBelirleyicisi() {
		return dosyaBelirleyicisi;
	}

	public void setDosyaBelirleyicisi(String dosyaBelirleyicisi) {
		this.dosyaBelirleyicisi = dosyaBelirleyicisi;
	}

	public String getDosyaTipi() {
		return dosyaTipi;
	}

	public void setDosyaTipi(String dosyaTipi) {
		this.dosyaTipi = dosyaTipi;
	}

	public String getDigerAlcakAciklama() {
		return digerAlcakAciklama;
	}

	public void setDigerAlcakAciklama(String digerAlcakAciklama) {
		this.digerAlcakAciklama = digerAlcakAciklama;
	}

	public String getParaBirimi() {
		return paraBirimi;
	}

	public void setParaBirimi(String paraBirimi) {
		this.paraBirimi = paraBirimi;
	}

	public String getTakipTarihi() {
		return takipTarihi;
	}

	public void setTakipTarihi(String takipTarihi) {
		this.takipTarihi = takipTarihi;
	}

	public String getUyapParaTuru() {
		return uyapParaTuru;
	}

	public void setUyapParaTuru(String uyapParaTuru) {
		this.uyapParaTuru = uyapParaTuru;
	}

	public String getBelgeMiktari() {
		return belgeMiktari;
	}

	public void setBelgeMiktari(String belgeMiktari) {
		this.belgeMiktari = belgeMiktari;
	}

	public String getDovizTipi() {
		return dovizTipi;
	}

	public void setDovizTipi(String dovizTipi) {
		this.dovizTipi = dovizTipi;
	}

	public String getAdSoyad() {
		return adSoyad;
	}

	public void setAdSoyad(String adSoyad) {
		this.adSoyad = adSoyad;
	}

	public String getAdresTuruId() {
		return adresTuruId;
	}

	public void setAdresTuruId(String adresTuruId) {
		this.adresTuruId = adresTuruId;
	}

	public String getAdres() {
		return adres;
	}

	public void setAdres(String adres) {
		this.adres = adres;
	}

	public String getAd() {
		return ad;
	}

	public void setAd(String ad) {
		this.ad = ad;
	}

	public String getSoyad() {
		return soyad;
	}

	public void setSoyad(String soyad) {
		this.soyad = soyad;
	}

	public String getTcNo() {
		return tcNo;
	}

	public void setTcNo(String tcNo) {
		this.tcNo = tcNo;
	}

	public String getIlAdi() {
		return ilAdi;
	}

	public void setIlAdi(String ilAdi) {
		this.ilAdi = ilAdi;
	}

	public String getIlId() {
		return ilId;
	}

	public void setIlId(String ilId) {
		this.ilId = ilId;
	}

	public ArrayList<FreeXMLHesap> getHesapListesi() {
		return hesapListesi;
	}

	public void setHesapListesi(ArrayList<FreeXMLHesap> hesapListesi) {
		this.hesapListesi = hesapListesi;
	}
	
	
	
	
}
